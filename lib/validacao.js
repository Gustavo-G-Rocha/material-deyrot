...
export function validarPedido(body) {
  ...
  // ---- etapa 3: adesivos --------------------------------------------------
  // Sim ou não, e no máximo um de cada por pedido: a quantidade não vem do formulário.
  const escolha = (campo, lista, msg) => {
    d[campo] = texto(body[campo]);
    if (!valoresDe(lista).includes(d[campo])) erros[campo] = msg;
  };
  escolha('adesivo_carro', opcoes.adesivoCarro, 'Diga se você quer o perfurado de vidro traseiro.');
  d.qtd_carros = d.adesivo_carro === 'sim' ? 1 : 0;

  escolha('adesivo_parachoque', opcoes.adesivoParachoque, 'Diga se você quer o adesivo de parachoque.');
