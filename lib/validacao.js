...
export function validarPedido(body) {
  ...
  // ---- etapa 3: perfurado de vidro traseiro ------------------------------
  // Sim ou não, e no máximo um por pedido: a quantidade não vem do formulário.
  d.adesivo_carro = texto(body.adesivo_carro);
  if (!valoresDe(opcoes.adesivoCarro).includes(d.adesivo_carro)) {
    erros.adesivo_carro = 'Diga se você quer o perfurado de vidro traseiro.';
  }
