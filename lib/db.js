...
export async function migrar() {
  await pool.query(`
    CREATE TABLE IF NOT EXISTS pedidos (
      ...
      -- etapa 3: adesivos ('sim'/'nao'; qtd é sempre 0 ou 1)
      adesivo_carro       TEXT        NOT NULL,
      qtd_carros          SMALLINT    NOT NULL DEFAULT 0,
      adesivo_parachoque  TEXT        NOT NULL DEFAULT 'nao',
      qtd_parachoques     SMALLINT    NOT NULL DEFAULT 0,

      -- etapa 4: alcance estimado (substituiu o perfil de engajamento abaixo)
      alcance             TEXT,

      /*
       * Perfil de engajamento: o formulário parou de perguntar (ver etapa 4
       * acima). As colunas ficam (o histórico não se joga fora), mas param de
       * ser preenchidas — a obrigatoriedade cai, senão todo INSERT novo quebra.
       * Elas também somem do CSV, veja COLUNAS_LEGADO.
       */
      disponibilidade     TEXT,
      contatos            TEXT,
      distribuidores      TEXT,
      mora_condominio     TEXT,
      unidades_condominio INTEGER,

      -- kit: sem escolha do usuário, todo pedido sai como 'm' (ver etapa 4)
      kit                 TEXT        NOT NULL,
      kit_recomendado     TEXT        NOT NULL,
      engajamento         SMALLINT    NOT NULL DEFAULT 0,

      -- etapa 5: consentimento e operação
      ...
      -- host que serviu o formulário: um app, dois domínios (um por candidato)
      origem              TEXT,
  ...
  /*
   * Bancos criados quando o formulário ainda perguntava sobre adesivo de moto.
   * As colunas ficam (o histórico não se joga fora), mas param de ser
   * preenchidas — então a obrigatoriedade precisa cair, senão todo INSERT novo
   * quebra. Elas também são omitidas do CSV, veja COLUNAS_LEGADO.
   */
  await pool.query(`
    DO $$ BEGIN
      IF EXISTS (SELECT 1 FROM information_schema.columns
                  WHERE table_name = 'pedidos' AND column_name = 'adesivo_moto') THEN
        ALTER TABLE pedidos ALTER COLUMN adesivo_moto DROP NOT NULL;
      END IF;
      IF EXISTS (SELECT 1 FROM information_schema.columns
                  WHERE table_name = 'pedidos' AND column_name = 'qtd_motos') THEN
        ALTER TABLE pedidos ALTER COLUMN qtd_motos DROP NOT NULL;
      END IF;
    END $$;
  `);

  /*
   * Bancos criados quando o formulário ainda perguntava o perfil de
   * engajamento (etapa 4 antiga). Mesma lógica do adesivo de moto acima:
   * as colunas ficam, mas a obrigatoriedade cai, senão todo INSERT novo quebra.
   */
  await pool.query(`
    ALTER TABLE pedidos
      ALTER COLUMN disponibilidade DROP NOT NULL,
      ALTER COLUMN contatos DROP NOT NULL,
      ALTER COLUMN distribuidores DROP NOT NULL,
      ALTER COLUMN mora_condominio DROP NOT NULL;
  `);

  // Bancos de antes do adesivo de parachoque e do alcance existirem.
  await pool.query(`ALTER TABLE pedidos ADD COLUMN IF NOT EXISTS adesivo_parachoque TEXT NOT NULL DEFAULT 'nao'`);
  await pool.query(`ALTER TABLE pedidos ADD COLUMN IF NOT EXISTS qtd_parachoques    SMALLINT NOT NULL DEFAULT 0`);
  await pool.query(`ALTER TABLE pedidos ADD COLUMN IF NOT EXISTS alcance            TEXT`);

  /*
   * Bancos de antes de o pedido registrar de qual domínio veio. Fica NULL nos
   * pedidos antigos e não há como preencher depois: o host não era guardado em
   * lugar nenhum, nem no banco nem no log da aplicação. NULL aqui quer dizer
   * "não dá para saber", e não "veio de um domínio qualquer".
   */
  await pool.query(`ALTER TABLE pedidos ADD COLUMN IF NOT EXISTS origem             TEXT`);
...
const COLUNAS = [
  ...
  'adesivo_carro', 'qtd_carros', 'adesivo_parachoque', 'qtd_parachoques',
  'alcance',
  'kit', 'kit_recomendado', 'engajamento',
  'aceite_lgpd', 'ip', 'user_agent', 'origem',
];
...
export async function listarPedidos({
  limite = 200, offset = 0, status = null, revisao = null, origem = null,
} = {}) {
  const filtros = [];
  const valores = [];
  ...
  // 'sem-origem' junta os pedidos antigos, de antes de o host ser gravado.
  if (origem === 'sem-origem') filtros.push('t.origem IS NULL');
  else if (origem) filtros.push(`t.origem = $${valores.push(origem)}`);
...
export async function contarPedidos() {
  const [total, porKit, porUf, perfurados, porRevisao, sinais, porOrigem] = await Promise.all([
    ...
    // 'quero' é o valor dos pedidos antigos, quando a pergunta tinha 4 opções.
    pool.query(`
      SELECT COUNT(*) FILTER (WHERE adesivo_carro IN ('sim', 'quero'))::int AS perfurados
      FROM pedidos`),
    ...
    pool.query(`
      ...
      FROM (${SQL_SINAIS}) t`),
    // NULL = pedido anterior ao registro de origem; some no COALESCE abaixo.
    pool.query(`
      SELECT COALESCE(origem, 'sem-origem') AS origem, COUNT(*)::int AS n
  ...
  return {
    ...
    porKit: porKit.rows,
    porUf: porUf.rows,
    perfurados: perfurados.rows[0].perfurados,
    revisao: {
...
/**
 * Corrige de qual domínio veio um pedido.
 *
 * Serve para os pedidos anteriores ao registro automático: o host não foi
 * guardado na época, então a origem deles só entra aqui, vinda de alguma
 * fonte externa (log do Railway, data de entrada do domínio, ou a própria
 * pessoa respondendo). `origem` nula devolve o pedido para "não sei".
 */
export async function atualizarOrigem(id, origem) {
  await pool.query(`UPDATE pedidos SET origem = $1 WHERE id = $2`, [origem, id]);
}

/** Mesma coisa, em lote: [{ id, origem }]. Devolve quantas linhas mudaram. */
export async function atualizarOrigemEmLote(pares) {
  if (!pares.length) return 0;
  const { rowCount } = await pool.query(
    `UPDATE pedidos AS p SET origem = v.origem
       FROM (SELECT unnest($1::int[]) AS id, unnest($2::text[]) AS origem) AS v
      WHERE p.id = v.id`,
    [pares.map((x) => x.id), pares.map((x) => x.origem)]);
  return rowCount;
}
 ...
 */
/** Colunas que ainda existem no banco por causa do histórico, mas não vão no CSV. */
const COLUNAS_LEGADO = new Set([
  'adesivo_moto', 'qtd_motos',
...
export async function exportarCsv() {
  ...
  const colunas = fields
    .map((f) => f.name)
    .filter((c) => c !== 'envio' && !COLUNAS_LEGADO.has(c));
  const cabecalho = [
    ...colunas,
    ...SLUGS.map((s) => `env_${s}`),
    'envio_editado',
